...
/*functions used at sphere part and triangle part*/
Ray createReflectRay(Vector normal, Vector intersection, Vector lightDir) {
	Vector direction = Vector(-lightDir.x, -lightDir.y, -lightDir.z);
	Vector reflectDir = normal * (direction.dot(normal));
	reflectDir = (reflectDir * 2 - direction).normalize();
	Vector reflect_origin = intersection + reflectDir * 0.001f ; // avoid the same intersection
	return Ray(reflect_origin, reflectDir);
}
...
Color createSphereShading(Sphere& sphere, Light& light, Vector& intersection) {
	...
	Vector lightPos = Vector(light.position[0], light.position[1], light.position[2]);
	//unit vectors used for phong shading
	Vector normal = (intersection - center).normalize();
	Vector lightDir = (lightPos - intersection).normalize();
	Vector reflectDir = normal * (lightDir.dot(normal));
	reflectDir = (reflectDir * 2 - lightDir).normalize();
	Vector eyeDir = Vector(-intersection.x, -intersection.y, -intersection.z);
	eyeDir.normalize();
	//clamp final result to 0 and 1.
	double diffuse_factor = lightDir.dot(normal);
	if (diffuse_factor > 1.0f) diffuse_factor = 1.0f;
	else if (diffuse_factor < 0.0f) diffuse_factor = 0.0f;
	double specular_factor = reflectDir.dot(eyeDir);
	if (specular_factor > 1.0f) specular_factor = 1.0f;
	else if (specular_factor < 0.0f) specular_factor = 0.0f;
...
bool Ray::intersectSphere(const Sphere& sphere, Vector& intersection, double& distance) {
	...
	double a = direction.dot(direction); // direction has been normalized
	...
	if (std::abs(delta) <= 1e-15) {
		...
		//This formula insures that the quantities added for q have the same sign, avoiding catastropic cancellation.
		//more stable when implemented on computers.
		double q = (b > 0) ? -0.5f*(b + sqrt(delta)) : -0.5f*(b - sqrt(delta));
		t0 = q / a;
		t1 = c / q;
	}
	if (t0 > t1) { // let t0 be the smaller one
		double tmp = t0;
		t0 = t1;
		t1 = tmp;
	}
...
Color detectSphereIntersection(Ray& ray, Color& color, double& closest_dist) {
	...
	// indicate the index of sphere, which is intersected with ray and has minimum distance
	index_sphere = -1;
	...
	for (int j = 0; j < num_lights; ++j) {
		Vector lightPos(lights[j].position[0], lights[j].position[1], lights[j].position[2]);
		Vector lightDir = (lightPos - sphere_intersection).normalize();
		Ray shadow(sphere_intersection, lightDir); // shadow ray
		...
		for (int k = 0; k < num_spheres; ++k) {
			...
			if (shadow.intersectSphere(spheres[k], shadow_intersection, distance) && k != index_sphere) {
				...
				Vector dist_light = lightPos - sphere_intersection;
...
Vector normalAtTriangle(Triangle triangle, Vector intersection) {
	...
	Vector vA = Vector(triangle.v[0].position[0], triangle.v[0].position[1], triangle.v[0].position[2]);
	Vector vB = Vector(triangle.v[1].position[0], triangle.v[1].position[1], triangle.v[1].position[2]);
	Vector vC = Vector(triangle.v[2].position[0], triangle.v[2].position[1], triangle.v[2].position[2]);
	Vector vD = intersection;

	Vector ABC_normal = (vB - vA).cross(vC - vA);
	double area_ABC = 0.5f * ABC_normal.magnitude();

	Vector DBC_normal = (vB - vD).cross(vC - vD);
	double area_DBC = 0.5f * DBC_normal.magnitude();

	Vector ADC_normal = (vD - vA).cross(vC - vA);
	double area_ADC = 0.5f * ADC_normal.magnitude();
	double alpha = area_DBC / area_ABC;
	double beta = area_ADC / area_ABC;
...
bool Ray::intersectTriangle(const Triangle& triangle, Vector& intersection, double& distance) {
	Vector vA(triangle.v[0].position[0], triangle.v[0].position[1], triangle.v[0].position[2]);
	Vector vB(triangle.v[1].position[0], triangle.v[1].position[1], triangle.v[1].position[2]);
	Vector vC(triangle.v[2].position[0], triangle.v[2].position[1], triangle.v[2].position[2]);

	Vector normal = (vB - vA).cross(vC - vA);
	normal.normalize();
	...
	double d = -(vA.dot(normal));// vA(x,y,z),normal(a,b,c), ax + by + cz + d = 0;
	...
	if (normal.dot((vB - vA).cross(intersection - vA)) < 0 ||
		normal.dot((vC - vB).cross(intersection - vB)) < 0 ||
		normal.dot((vA - vC).cross(intersection - vC)) < 0)
	{
		return false;
	}
	return true;
...
Color detectTriangleIntersection(Ray& ray, Color& color, double& closest_dist) {
	...
	for (int j = 0; j < num_lights; ++j) {
		Vector lightPos(lights[j].position[0], lights[j].position[1], lights[j].position[2]);
		Vector lightDir = (lightPos - triangle_intersection).normalize();
		Ray shadow(triangle_intersection, lightDir); // shadow ray
		...
		for (int k = 0; k < num_spheres; ++k) {
			...
			if (shadow.intersectSphere(spheres[k], shadow_intersection, distance)) {
				...
				Vector dist_light = lightPos - triangle_intersection;
				// Make sure that the shadow intersection point does not pass the light
				if (dist_shadow.magnitude() < dist_light.magnitude()) {
					hasLight = false;
					break;
...
Color createColor(Ray& ray, int count_reflect) {
	if (count_reflect > reflect_time) {
		return Color(0, 0, 0);
	}
	Color color;
	Color litColor, reflectColor;
	double closest_dist = INFINITY;

	litColor = detectSphereIntersection(ray, litColor, closest_dist);
	litColor = detectTriangleIntersection(ray, litColor, closest_dist);
	...
	if (index_sphere != -1) {
		...
		double reflect_dist;
		reflectColor = createColor(reflectRay, count_reflect + 1);
	}
	//triangle
	if (index_triangle != -1) {
